...
Port EBh: page register RAM 2: bit 7 = 1 => enable EMS page mapping for the selected memory block location in the index register E8h.  
...
- soldering the memory modules is best done with solid core component pin wires or at least some type of solid core tinned wire. The wires can be run to both sides of the module and bent down from both sides to meet the holes of the modules on the mainboard side. A sufficiently powerful soldering iron with thick wedge tip to conduct heat well and leaded solder and perhaps some no clean flux should be capable of soldering the wires. Some thick type of paper strips are recommended to separate the soldered ends sticking out on the module to short with the other row bent down across them. Clipping off those ends neatly so they don't stick out far is also recommended but caution must be observed to not damage anything. Be careful to observe the correct orientation and correct pin rows going into the correct matchine ones on the mainboard. (!) Angled pin headers in the 3d view pictures should not be used, I tried but decided against keeping these because of tightness of the modules when using them and also being more difficult to solder in using the iron. This board is particularly heavy on the ground and power planes so it's too efficient to conduct away the heat of soldering. So using the component wires, this needs a whole lot less heat and duration of the soldering. Make sure to flow the solder well and check your work using magnification after soldering in each module, before continuing to the next one so you can still inspect it closely without any hampering.
